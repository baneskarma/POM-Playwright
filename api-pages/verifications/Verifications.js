...
export class Verifications {
  ...
  async verifyResponse( response, statusCode ) {
    await allure.step(
      "Response is ok and status code is " + statusCode.toString(),
      async () => {
  ...
  async verifyObjectIdAndData( objectType, objectId, objectUrl, objectData ) {
    await allure.step(
      "Verify " + objectType + " was successfully created or updated",
      async () => {
        let response;
        let responseObjectData;
        try {
          response = await this.request.get( objectUrl, {headers: {Authorization: `Bearer ${process.env.sfAccessToken}`,},});
          ...
          await allure.step(
            "Verify " + objectType + " has ID: " + objectId,
            async () => {
  ...
  async verifyAccountData( responseData, objectData ) {
    await allure.step( "Verify Account Data", async () => {
      if (
        ...
        objectData.name.trim().length > 0
      ) {
        await allure.step(
          "Verify Account Name is: " + objectData.name,
          async () => {
      ...
      if (
        ...
        objectData.type.trim().length > 0
      ) {
        await allure.step(
          "Verify Account Type is: " + objectData.type,
          async () => {
      ...
      if (
        typeof objectData.phone === "string" &&
        objectData.phone.trim().length > 0
      ) {
        await allure.step(
          "Verify Account Phone is: " + objectData.phone,
          async () => {
      ...
      if (
        typeof objectData.billingStreet === "string" &&
        objectData.billingStreet.trim().length > 0
      ) {
        await allure.step(
          "Verify Account billing street is: " + objectData.billingStreet,
          async () => {
      ...
      if (
        typeof objectData.billingPostalCode === "string" &&
        objectData.billingPostalCode.trim().length > 0
      ) {
        await allure.step(
          "Verify Account billing postal code is: " +
            objectData.billingPostalCode,
          async () => {
            expect( responseData.BillingPostalCode ).toBe(
              objectData.billingPostalCode,
            );
          },
        );
      ...
      if (
        typeof objectData.billingCity === "string" &&
        objectData.billingCity.trim().length > 0
      ) {
        await allure.step(
          "Verify Account billing city is: " + objectData.billingCity,
          async () => {
      ...
      if (
        typeof objectData.billingProvince === "string" &&
        objectData.billingProvince.trim().length > 0
      ) {
        await allure.step(
          "Verify Account billing province is: " + objectData.billingProvince,
          async () => {
            expect( responseData.BillingProvince ).toBe(
              objectData.billingProvince,
            );
          },
        );
      }
      ...
      if (
        typeof objectData.billingCountry === "string" &&
        objectData.billingCountry.trim().length > 0
      ) {
        await allure.step(
          "Verify Account billing country is: " + objectData.billingCountry,
          async () => {
  ...
  async verifyOrderData( responseData, objectData ) {
    await allure.step( "Verify Order Data", async () => {
      if (
        typeof objectData.AccountId === "string" &&
        objectData.AccountId.trim().length > 0
      ) {
        await allure.step(
          "Verify Order Account ID is: " + objectData.AccountId,
          async () => {
            expect( responseData ).toHaveProperty(
              "AccountId",
              objectData.AccountId,
            );
      ...
      if (
        typeof objectData.EffectiveDate === "string" &&
        objectData.EffectiveDate.trim().length > 0
      ) {
        await allure.step(
          "Verify Order Start Date is: " + objectData.EffectiveDate,
          async () => {
            expect( responseData ).toHaveProperty(
              "EffectiveDate",
              objectData.EffectiveDate,
            );
          },
        );
      }
      ...
      if (
        ...
        objectData.Status.trim().length > 0
      ) {
        await allure.step(
          "Verify Order Status is: " + objectData.Status,
          async () => {
      ...
      if (
        typeof objectData.vlocity_cmt__PriceListId__c === "string" &&
        objectData.vlocity_cmt__PriceListId__c.trim().length > 0
      ) {
        await allure.step(
          "Verify Order Price list is " +
            objectData.priceListName +
            ", with id: " +
            objectData.vlocity_cmt__PriceListId__c,
          async () => {
            expect( responseData ).toHaveProperty(
      ...
      if (
        typeof objectData.discountName === "string" &&
        objectData.discountName.trim().length > 0
      ) {
        await allure.step(
          "Verify Order has Discount '" +
            objectData.discountName +
            "', with id: " +
            objectData.vlocity_cmt__Discount__c,
          async () => {
            expect( responseData ).toHaveProperty(
  ...
  async verifyOrderItemData( responseData, objectData ) {
    await allure.step( "Verify Order Item Data", async () => {
      ...
      await allure.step(
        "Verify Order Item has Order with ID: " + objectData.OrderId,
        async () => {
      ...
      await allure.step(
        "Verify Order Item has product with ID: " + objectData.Product2Id,
        async () => {
          expect( responseData ).toHaveProperty(
            "Product2Id",
            objectData.Product2Id,
          );
        },
      );

      await allure.step(
        "Verify Order Item has PricebookEntry ID: " +
          objectData.PricebookEntryId,
        async () => {
          expect( responseData ).toHaveProperty(
            "PricebookEntryId",
            objectData.PricebookEntryId,
      ...
      await allure.step(
        "Verify Order Item has Quantity of: " + objectData.Quantity,
        async () => {
      ...
      await allure.step(
        "Verify Order Item has UnitPrice of: " + objectData.UnitPrice,
        async () => {
      ...
      await allure.step(
        "Verify Order Item has Total Price of: " + totalPrice,
        async () => {
  ...
  async verifyAssetData( responseData, objectData ) {
    await allure.step( "Verify Asset Data", async () => {
      ...
      await allure.step(
        "Verify Asset has name: " + objectData.Name,
        async () => {
          expect( responseData ).toHaveProperty( "Name", objectData.Name );
          expect( responseData.Name ).toBe( objectData.Name );
        },
      ...
      await allure.step(
        "Verify Asset has product with ID: " + objectData.Product2Id,
        async () => {
          expect( responseData ).toHaveProperty(
            "Product2Id",
            objectData.Product2Id,
      ...
      await allure.step(
        "Verify Asset has account with ID: " + objectData.AccountId,
        async () => {
          expect( responseData ).toHaveProperty(
            "AccountId",
            objectData.AccountId,
      ...
      await allure.step(
        "Verify Asset has Quantity of: " + objectData.Quantity,
        async () => {
          expect( responseData ).toHaveProperty( "Quantity", quantity );
        },
      );
      ...
      await allure.step(
        "Verify Asset has Price of: " + objectData.Price,
        async () => {
