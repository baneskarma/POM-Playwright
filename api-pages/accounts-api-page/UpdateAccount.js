	...
	async billingAddress(accountId, accountUrl, billingData) {
		...
		await allure.step('Update account', async () => {
			...
			if (typeof billingData.billingStreet === 'string' && billingData.billingStreet.trim().length > 0) {
				await allure.step('Add billing street: ' + billingData.billingStreet, async () => {
			...
			if (typeof billingData.billingPostalCode === 'string' && billingData.billingPostalCode.trim().length > 0) {
				await allure.step('Add billing postal code: ' + billingData.billingPostalCode, async () => {
			...
			if (typeof billingData.billingCity === 'string' && billingData.billingCity.trim().length > 0) {
				await allure.step('Add billing city: ' + billingData.billingCity, async () => {
			...
			if (typeof billingData.billingProvince === 'string' && billingData.billingProvince.trim().length > 0) {
				await allure.step('Add billing province: ' + billingData.billingProvince, async () => {
			...
			if (typeof billingData.billingCountry === 'string' && billingData.billingCountry.trim().length > 0) {
				await allure.step('Add billing country: ' + billingData.billingCountry, async () => {
			...
			try {
				...
				await verifications.verifyObjectIdAndData('Account', accountId, accountUrl, accountData);
			} catch (error) {
