...
export class CreateAsset {
   ...
   *   It also stores the id and url from the created asset in environment variables. process.env.currentAssetUrl and process.env.assetId
   *
  ...
  async newAsset( accountId, productName, price, quantity ) {
    await allure.step( "Create new Asset", async () => {
      ...
      try {
        const response = await this.request.post( process.env.commonAssetUrl, {
          headers: {
        ...
        await allure.step( "Retrieving response data", async () => {
          ...
          await allure.step( "Store Asset ID", async () => {
            ...
            process.env.assetId = assetId;
            //console.log("Asset ID: " + process.env.assetId);
          });

          await allure.step( "Store Asset Url", async () => {
            const assetUrl = process.env.commonAssetUrl + process.env.assetId;
            process.env.currentAssetUrl = assetUrl;
          });
        });
    ...
    await allure.step( "Get id for the product: " + productName, async () => {
      try {
        const response = await this.request.get(
          process.env.commonUrl + "query?q=SELECT+Name,Id+FROM+Product2",
