...
export class VerifyProduct {
	...
	async verifyProductExists(productName, productCode, childProductBool) {
		...
		let responseData;
		let records;
		await allure.step("Verify product: '" + productName + "' exists with product code: " + productCode, async () => {
			try {
				const response = await this.request.get(process.env.COMMON_URL + 'query?q=SELECT+Name,ProductCode,Id+FROM+Product2', {
					headers: {
				...
				for (let i = 0; i < records.length; i++) {
					if (records[i].Name === productName && records[i].ProductCode === productCode) {
						productId = records[i].Id;

						if (!childProductBool) {
							await allure.step("Store the product's id: " + productId, async () => {
							...
							await allure.step('Store the product url', async () => {
								process.env.CURRENT_PRODUCT_URL = process.env.COMMON_PRODUCT_URL + process.env.PRODUCT_ID;
							});
						} else {
							await allure.step("Return the child product's id: " + productId, async () => {
								return productId;
							});
						}
						...
						throw new Error("There is no product with name: '" + productName + "', and product code: " + productCode);
					}
				}
			} catch (error) {
				await allure.step('Verifying existence of product has failed: ' + error, async () => {
					throw error;
				});
	...
	async verifyProductDetails(productUrl, productData) {
		await allure.step('Verify details of product: ' + productData.Name, async () => {
			let responseData;
			try {
				const response = await this.request.get(productUrl, {
					headers: {
						Authorization: `Bearer ${process.env.ACCESS_TOKEN}`,
					},
				});
				...
				if (typeof productData.Name === 'string' && productData.Name.trim().length > 0) {
					await allure.step("Verify product's name is: " + productData.Name, async () => {
						expect(responseData.Name).toBe(productData.Name);
					});
				}

				if (typeof productData.ProductCode === 'string' && productData.ProductCode.trim().length > 0) {
					await allure.step("Verify product's code is: " + productData.ProductCode, async () => {
						expect(responseData.ProductCode).toBe(productData.ProductCode);
					});
				}

				if (typeof productData.Family === 'string' && productData.Family.trim().length > 0) {
					await allure.step('Verify product family is: ' + productData.Family, async () => {
						expect(responseData.Family).toBe(productData.Family);
					});
				}

				if (typeof productData.Customer_Type__c === 'string' && productData.Customer_Type__c.trim().length > 0) {
					await allure.step("Verify product's customer type is: " + productData.Customer_Type__c, async () => {
						expect(responseData.Customer_Type__c).toBe(productData.Customer_Type__c);
					});
				}

				if (typeof productData.Charging__c === 'string' && productData.Charging__c.trim().length > 0) {
					await allure.step("Verify product's charging is: " + productData.Charging__c, async () => {
						expect(responseData.Charging__c).toBe(productData.Charging__c);
					});
				}

				if (typeof productData.PackageType === 'string' && productData.PackageType.trim().length > 0) {
					await allure.step("Verify product's package type is: " + productData.PackageType, async () => {
						//const responseAttributes = await responseData.vlocity_cmt__AttributeDefaultValues__c;
						const responseAttributes = await JSON.parse(responseData.vlocity_cmt__AttributeDefaultValues__c);
				...
				await allure.step('Verifying details of product failed: ' + error, async () => {
					throw error;
				});
	 ...
	 */
	async verifyChildProducts(parentProductName, parentProductId, childProductNames, childProductCodes) {
		await allure.step('Verify child products for product: ' + parentProductName, async () => {
			...
			try {
				await allure.step('Verify child products existence', async () => {
					for (let i = 0; i < childProductNames.length; i++) {
						...
						childProductId = await this.verifyProductExists(currentChildProductName, currentChildProductCode, true);
						childProductIds.push(childProductId);
					}
				});

				await allure.step('Get all child product items', async () => {
					const response = await this.request.get(process.env.COMMON_URL + 'query?q=SELECT+vlocity_cmt__ChildProductId__c,vlocity_cmt__ParentProductId__c,vlocity_cmt__ChildProductName__c,vlocity_cmt__ParentProductName__c+FROM+vlocity_cmt__ProductChildItem__c', {
						headers: {
							Authorization: `Bearer ${process.env.ACCESS_TOKEN}`,
						},
					});
				...
				for (let i = 0; i < childProductIds.length; i++) {
					...
					await allure.step("Verify product '" + currentChildProductName + "', is a child product of " + parentProductName, async () => {
						for (let j = 0; j < records.length; j++) {
							if (currentChildProductId === records[j].vlocity_cmt__ChildProductId__c && parentProductId === records[j].vlocity_cmt__ParentProductId__c) {
								expect(records[j]).toHaveProperty('vlocity_cmt__ChildProductName__c', currentChildProductName);
								expect(records[j]).toHaveProperty('vlocity_cmt__ParentProductName__c', parentProductName);
								break;
							} else if (j === records.length - 1) {
								throw new Error("There is no child product with name: '" + currentChildProductName + "', for product: '" + parentProductName + "', with id: " + parentProductId);
							}
						}
					});
				}
