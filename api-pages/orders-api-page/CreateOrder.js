...
export class CreateOrder {
   ...
   *   It also stores the id and url from the created order in environment variables. **process.env.currentOrderUrl** and **process.env.orderId**
   *
  ...
  async newOrder( accountId, startDate, priceListName, status ) {
    await allure.step( "Create new Order", async () => {
      ...
      try {
        const response = await this.request.post( process.env.commonOrderUrl, {
          headers: {
        ...
        await allure.step( "Retrieving response data", async () => {
          ...
          await allure.step( "Store Order ID", async () => {
            ...
            process.env.orderId = orderId;
            //console.log("Order ID: " + process.env.orderId);
          });

          await allure.step( "Store Order Url", async () => {
            orderUrl = process.env.commonOrderUrl + process.env.orderId;
            process.env.currentOrderUrl = orderUrl;
          });
        });
  ...
  async getPriceListId( priceListName ) {
    let priceListId;
    await allure.step(
      "Get id for the price list: " + priceListName,
      async () => {
        try {
          const response = await this.request.get(
            process.env.commonUrl +
              "query?q=SELECT+Name,Id+FROM+vlocity_cmt__PriceList__c",
  ...
  async createOrderItem( orderId, productName, quantity, unitPrice ) {
    ...
    await allure.step(
      "Add Product '" + productName + "' to Order",
      async () => {
        let orderItemData;
        let orderItemUrl;

        const productAndPricebookEntryId =
          await this.getProductIdAndPricebookEntryId( productName );
        ...
        await allure.step( "Store Order Item Data", async () => {
          orderItemData = {
            ...
            //"vlocity_cmt__AssetId__c": process.env.assetId,
          };
        });

        try {
          const response = await this.request.post(
            process.env.commonOrderItemUrl,
            {
              headers: {
                Authorization: `Bearer ${process.env.sfAccessToken}`,
                "Content-Type": "application/json",
          ...
          await allure.step( "Retrieving response data", async () => {
            const responseData = await response.json();
            console.log(
              "Response from adding item to order is Success:",
              responseData,
            );

            await allure.step( "Store Order Item ID", async () => {
              const orderItemId = await responseData.id;
              process.env.orderItemId = orderItemId;
              //console.log("Order ID: " + process.env.orderId);
            });

            await allure.step( "Store Order Item Url", async () => {
              orderItemUrl =
                process.env.commonOrderItemUrl + process.env.orderItemId;
              process.env.currentOrderItemUrl = orderItemUrl;
            });
          });

          await this.verifications.verifyObjectIdAndData(
            "OrderItem",
            process.env.orderItemId,
            process.env.currentOrderItemUrl,
            orderItemData,
          );
        } catch ( error ) {
  ...
  async getProductIdAndPricebookEntryId( productName ) {
    ...
    await allure.step(
      "Get id for the product and pricebook entry: " + productName,
      async () => {
        try {
          const response = await this.request.get(
            process.env.commonUrl +
              "query?q=SELECT+Name,Id,Product2Id+FROM+PriceBookEntry",
            {headers: {Authorization: `Bearer ${process.env.sfAccessToken}`,},},
          );
          ...
          throw new Error(
            "There is no pricebook entry with name: " + productName,
          );
        } catch ( error ) {
          console.error(
            "Getting Product ID and Pricebook Entry ID failed:",
            error,
          );
          throw error;
        }
  ...
  async changeOrderStatus( orderStatus, orderUrl ) {
    await allure.step( "Change order status to: " + orderStatus, async () => {
      try {
        ...
        console.error(
          "Change order status to '" + orderStatus + "', has failed: ",
          error,
        );
        throw error;
      }
  ...
  async getDiscountId( discountName ) {
    ...
    await allure.step( "Get id for the discount: " + discountName, async () => {
      try {
        const response = await this.request.get(
          process.env.commonUrl +
            "query?q=SELECT+Name,Id+FROM+vlocity_cmt__OrderDiscount__c",
          {headers: {Authorization: `Bearer ${process.env.sfAccessToken}`,},},
        );
