...
export async function verify10Times10GBNational150kr() {
	test( 'API-Verify10Times10GBNational150kr', async ({ request }) => {
		allure.description( "Verify that product '10Times 10GB National 150kr' exists, and verify the product details and it's child products" );
		...
		const childProductCodes = [ 'PRD-FREE-NATIONAL-MOBILE-&-FIXED-600min', 'PRD-FREE-NATIONAL-DATA-100GB', 'PRD-FREE-100-SMS-&-MMS' ];
		...
		// Step 3:
		await verifyProduct.verifyChildProducts( productName, process.env.PRODUCT_ID, childProductNames, childProductCodes );
	});
}
