...
export async function zeroStepDemo() {
  test( "ZeroStepDemo", async () => {
    allure.description(
      "Verify that product '10Times 10GB National 150kr' exists, and to verify product details and it's child products",
    );
    ...
    // Step 2:
    await ai(
      "Click the 'Vlocity Product Console' link, and wait for the page to fully load",
      { test, page },
    );
    ...
    await ai(
      "On the table if there is a row with exact product name '10Times 10GB National 150kr' and on the same row it has column with this exact product code '1OFF-10TIMES-10GB-NATIONAL', click that row product name link",
      { test, page },
    );
    ...
    await ai(
      "Click the dropdown next to 'Customer Type', and choose 'Residential'",
      { test, page },
    );

    await ai( "Scroll down until field 'Not Assetizable' is into view", {
      test,
      page,
    });
    ...
    await ai(
      "To the DATETIME field, next to 'Selling End Date' write the value '02/05/2023', and click enter.",
      { test, page },
    );
