...
export async function order5GNetworkVoiceToBusiness() {
  test( "5GNetwork-VoiceToBusiness", async ( /**{homePage, createAccount, createOrder}*/ ) => {
    allure.description(
      "Order product '5G Network - Voice' through business account with B2B product price list.",
    );
    ...
    let totalCharges = (
      parseInt( quantity ) * parseInt( recurringCharges )
    ).toString();
    ...
    // Step 4:
    await createOrder.configureOrder(
      productName,
      quantity,
      chargesType,
      totalCharges,
