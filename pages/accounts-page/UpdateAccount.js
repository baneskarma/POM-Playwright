...
export class UpdateAccount {
  constructor( page ) {
    ( async () => {
      ...
      this.recentlyViewed = page.locator(
        "//span[text()='Accounts']//following-sibling::span[text()='Recently Viewed']",
      );
    ...
    await allure.step( "Search for account " + accountName, async () => {
      ...
      await allure.step( "Go to account " + accountName, async () => {
        await allure.step( "Click on the searched account", async () => {
          ...
          this.searchedAccount = this.page.locator(
            "//a[@title='" + accountName + "']",
          );
        ...
        await allure.step(
          "Click on searched account in recently viewed",
          async () => {
        ...
        await allure.step( "Verify account name", async () => {
          this.accountNameVerification = this.page
            .locator( "//h1//slot//*[text()='" + accountName + "']" )
            .last();
  ...
  async billingAddress(
    ...
    await allure.step( "Update billing address", async () => {
      ...
      if (
        typeof postalCodeValue === "string" &&
        postalCodeValue.trim().length > 0
      ) {
        await allure.step(
          "Update postal code value to " + postalCodeValue,
          async () => {
      ...
      if (
        ...
        provinceValue.trim().length > 0
      ) {
        await allure.step(
          "Update province value to " + provinceValue,
          async () => {
      ...
      if ( typeof countryValue === "string" && countryValue.trim().length > 0 ) {
        await allure.step(
          "Update country value to " + countryValue,
          async () => {
